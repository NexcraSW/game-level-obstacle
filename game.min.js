...
// lib/game/fsm.js
ig.baked=true;ig.module('game.fsm').defines(function()
{Fsm=ig.Class.extend({fsmOwner:null,prevState:null,currentState:null,globalState:null,init:function(settings)
{this.fsmOwner=settings.fsmOwner;this.currentState=settings.beginState;this.globalState=settings.globalState;if(this.globalState&&this.globalState.enter)
{this.globalState.enter(this.fsmOwner);}
if(this.currentState.enter)
{this.currentState.enter(this.fsmOwner);}},update:function(dt)
{ig.assert(this.currentState!=null,'Current state somehow is null during update!');if(this.globalState!=null&&this.globalState.update)
{this.globalState.update(this.fsmOwner,dt);}
if(this.currentState.update)
{this.currentState.update(this.fsmOwner,dt);}},changeState:function(state)
{ig.assert(this.currentState!=null,'Current state has gone missing during enter!');if(this.currentState.exit)
{this.currentState.exit(this.fsmOwner);}
this.prevState=this.currentState;this.currentState=state;if(this.currentState.enter)
{this.currentState.enter(this.fsmOwner);}},isInState:function(stateToCheck)
{ig.assert(this.stateStack.length>=1,'Entity ran out of states during isInState check!');var inState=false;if(this.currentState.state===stateToCheck.state)
{inState=true;}
return inState;},sendMessage:function(message,extraData)
{var msgHandled=false;if(this.currentState.onMessage)
...
// lib/game/hotFloor.js
ig.baked=true;ig.module('game.hotFloor').requires('game.fsm','impact.entity').defines(function(){EntityHotFloor=ig.Entity.extend({size:{x:200,y:200},entityFsm:null,openDur:0,openDurRange:{min:0.5,max:3.0},preFireDur:0,preFireDurTotal:1.0,fireDur:0,fireDurTotal:2.0,idleSheet:null,openSheet:null,fireSheet:null,init:function(x,y,settings)
{this.parent(x,y,settings);this.entityFsm=new Fsm({fsmOwner:this,beginState:HotFloorBeginState,globalState:HotFloorGlobalState});this.idleSheet=new ig.AnimationSheet('media/artAssets/stageHazards/pd-scenery-stage10-props-floorTrap1.png',200,200);this.openSheet=new ig.AnimationSheet('media/artAssets/stageHazards/pd-scenery-stage10-props-floorTrap2.png',200,200);this.fireSheet=new ig.AnimationSheet('media/artAssets/stageHazards/pd-scenery-stage10-props-floorTrap3.png',200,200);this.anims.idle=new ig.Animation(this.idleSheet,0.05,[0],true);this.anims.open=new ig.Animation(this.openSheet,0.05,[0],true);this.anims.fire=new ig.Animation(this.fireSheet,0.05,[0,1],false);this.currentAnim=this.anims.idle;},update:function()
{this.parent();this.entityFsm.update(ig.system.tick);},sendMessage:function(message,extraData)
{var msgHandled=this.entityFsm.sendMessage(message,extraData);return msgHandled;},});});ig.HOT_FLOOR_STATE={'GLOBAL':0,'BEGIN':1,'IDLE':2,'ACTIVE_CLOSED':3,'ACTIVE_OPEN':4,'FIRE':5,'INACTIVE':6,};ig.HOT_FLOOR_MSGS={'ACTIVATE':0,'DEACTIVATE':1,}
HotFloorGlobalState={state:ig.HOT_FLOOR_STATE.GLOBAL,onMessage:function(hotFloor,message,extraData)
{var msgHandled=true;switch(message)
{case ig.HOT_FLOOR_MSGS.DEACTIVATE:hotFloor.entityFsm.changeState(HotFloorInactiveState);break;default:msgHandled=false;break;}
return msgHandled;}}
HotFloorBeginState={state:ig.HOT_FLOOR_STATE.BEGIN,onMessage:function(hotFloor,message,extraData)
{var msgHandled=true;switch(message)
{case ig.HOT_FLOOR_MSGS.ACTIVATE:hotFloor.entityFsm.changeState(HotFloorIdleState);break;default:msgHandled=false;break;}
return msgHandled;}}
HotFloorIdleState={state:ig.HOT_FLOOR_STATE.IDLE,enter:function(hotFloor)
{hotFloor.currentAnim=hotFloor.anims.idle;},update:function(hotFloor,dt)
{hotFloor.entityFsm.changeState(HotFloorActiveClosedState);}}
HotFloorActiveClosedState={state:ig.HOT_FLOOR_STATE.ACTIVE_CLOSED,enter:function(hotFloor)
{hotFloor.currentAnim=hotFloor.anims.idle;hotFloor.openDur=(Math.random()*hotFloor.openDurRange.max-hotFloor.openDurRange.min)+hotFloor.openDurRange.min;},update:function(hotFloor,dt)
{hotFloor.openDur-=dt;if(hotFloor.openDur<=0)
{hotFloor.entityFsm.changeState(HotFloorActiveOpenState);}},}
HotFloorActiveOpenState={state:ig.HOT_FLOOR_STATE.ACTIVE_OPEN,enter:function(hotFloor)
{hotFloor.currentAnim=hotFloor.anims.open;hotFloor.preFireDur=hotFloor.preFireDurTotal;},update:function(hotFloor,dt)
{hotFloor.preFireDur-=dt;if(hotFloor.preFireDur<=0)
{hotFloor.entityFsm.changeState(HotFloorFireState);}},}
HotFloorFireState={state:ig.HOT_FLOOR_STATE.FIRE,enter:function(hotFloor)
{hotFloor.currentAnim=hotFloor.anims.fire;hotFloor.fireDur=hotFloor.fireDurTotal;},update:function(hotFloor,dt)
{hotFloor.fireDur-=dt;if(hotFloor.fireDur<=0)
{hotFloor.entityFsm.changeState(HotFloorActiveClosedState);}},}
HotFloorInactiveState={state:ig.HOT_FLOOR_STATE.INACTIVE,enter:function(hotFloor)
{hotFloor.currentAnim=hotFloor.anims.idle;},onMessage:function(hotFloor,message,extraData)
{var msgHandled=true;switch(message)
{case ig.HOT_FLOOR_MSGS.ACTIVATE:hotFloor.entityFsm.changeState(HotFloorIdleState);break;default:msgHandled=false;break;}
return msgHandled;}}
...
this.reversionCanvas=this.data;},draw:function(text,x,y,align){if(!this.loaded){return;}
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('game.hotFloor','impact.game','plugins.font2').defines(function()
{TrapDoorSample=ig.Game.extend({bgImg:new ig.Image('media/artAssets/background/pd-trapDoorDemo-bg.png'),trapDoorsTop:[],trapDoorsBottom:[],trapDoorsHorizGapDist:296,numTrapDoorsPerRow:3,trapDoorsActive:false,uiPanelBg:new ig.Image('media/ui/pd-trapDoorDemo-ui-backPanel.png'),uiPanelBgWidth:434,uiPanelCopyright:new ig.Image('media/ui/pd-trapDoorDemo-ui-copyrightNotice.png'),uiTextTitle:new ig.Font('media/ui/pd-trapDoorDemo-ui-font-pt36.png'),uiTextInstrActivate:new ig.Font('media/ui/pd-trapDoorDemo-ui-font-pt24.png'),uiTextInstrDeactivate:new ig.Font('media/ui/pd-trapDoorDemo-ui-font-pt24.png'),uiTextStatus:new ig.Font('media/ui/pd-trapDoorDemo-ui-font-pt36.png'),inputCodeActivate:'INPUT-KEY-ACTIVATE-TRAPDOORS',inputCodeDeactivate:'INPUT-KEY-DEACTIVATE-TRAPDOORS',init:function()
{ig.input.bind(ig.KEY._1,this.inputCodeActivate);ig.input.bind(ig.KEY._2,this.inputCodeDeactivate);var trapDoorsTopHorizStartPos={x:284,y:350};var trapDoorsBottomHorizStartPos={x:193,y:457};for(var i=0;i<this.numTrapDoorsPerRow;i++)
{this.trapDoorsTop.push(new EntityHotFloor(ig.game.screen.x+trapDoorsTopHorizStartPos.x+(i*this.trapDoorsHorizGapDist),ig.game.screen.y+trapDoorsTopHorizStartPos.y,null));this.trapDoorsBottom.push(new EntityHotFloor(ig.game.screen.x+trapDoorsBottomHorizStartPos.x+(i*this.trapDoorsHorizGapDist),ig.game.screen.y+trapDoorsBottomHorizStartPos.y,null));}},update:function()
{this.parent();if(ig.input.pressed(this.inputCodeActivate))
...
{this.trapDoorsTop[i].update();this.trapDoorsBottom[i].update();}},activateTrapDoors:function()
{if(!this.trapDoorsActive)
{for(var i=0;i<this.numTrapDoorsPerRow;i++)
{this.trapDoorsTop[i].sendMessage(ig.HOT_FLOOR_MSGS.ACTIVATE,null);this.trapDoorsBottom[i].sendMessage(ig.HOT_FLOOR_MSGS.ACTIVATE,null);}
this.trapDoorsActive=true;}},deactivateTrapDoors:function()
{if(this.trapDoorsActive)
{for(var i=0;i<this.numTrapDoorsPerRow;i++)
{this.trapDoorsTop[i].sendMessage(ig.HOT_FLOOR_MSGS.DEACTIVATE,null);this.trapDoorsBottom[i].sendMessage(ig.HOT_FLOOR_MSGS.DEACTIVATE,null);}
this.trapDoorsActive=false;}},draw:function()
{this.parent();var screenPosX=ig.game.screen.x;var screenPosY=ig.game.screen.y;var screenWidth=ig.system.width;var screenHeight=ig.system.height;this.bgImg.draw(screenPosX,screenPosY);for(var i=0;i<this.numTrapDoorsPerRow;i++)
{this.trapDoorsTop[i].draw();this.trapDoorsBottom[i].draw();}
this.uiPanelBg.draw((screenPosX+screenWidth*0.5)-(this.uiPanelBgWidth*0.5),screenPosY+(screenHeight*0.04));this.uiPanelCopyright.draw(screenPosX+(screenWidth*0.849),screenPosY+(screenHeight*0.962));this.uiTextTitle.draw('Trap Door Demo',screenPosX+screenWidth*0.5,screenPosY+(screenHeight*0.065),ig.Font.ALIGN.CENTER);this.uiTextInstrActivate.draw('Press \'1\' to activate',screenPosX+screenWidth*0.41,screenPosY+(screenHeight*0.156),ig.Font.ALIGN.LEFT);this.uiTextInstrDeactivate.draw('Press \'2\' to deactivate',screenPosX+screenWidth*0.41,screenPosY+(screenHeight*0.223),ig.Font.ALIGN.LEFT);if(this.trapDoorsActive)
...
{this.uiTextStatus.draw('Current Status: [#FF2600 INACTIVE]',screenPosX+screenWidth*0.5,screenPosY+(screenHeight*0.293),ig.Font.ALIGN.CENTER);}}});ig.main('#canvas',TrapDoorSample,60,1280,720,1);});